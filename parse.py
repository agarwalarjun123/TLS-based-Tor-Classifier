...
def parse(filename):
    ...
    for pkt in cap:
        if hasattr(pkt,'tcp') and hasattr(pkt, 'tls'):
            if not str(pkt.tcp.stream) in streams:
                stream_id = str(pkt.tcp.stream)
                streams[stream_id] = {"source_ip": pkt.ip.src, "dest_ip": pkt.ip.dst, "pkt_count": 1}
            ...
            if hasattr(pkt, 'tls'):
                streams[stream_id] = get_tls_payload(pkt.tls, streams[stream_id])
            print(streams[stream_id])                
    f = open('output1.csv','w',encoding='utf8')
    w = csv.DictWriter(f,['source_ip','dest_ip','pkt_count','tcp_src_port','tcp_dest_port','tls_cert_length','tls_issuer','tls_algorithm_id','tls_handshake_ciphersuite','tls_handshake_extensions_length','tls_handshake_server_curve_type','tls_handshake_server_named_curve','tls_handshake_echde_server_pubkey_len','tls_handshake_echde_client_pubkey_len','tls_server_name','tls_ja3_hash','tls_cipher_suite_length'])
    w.writeheader()
    for record in list(streams.values()):
        w.writerow(flatten(record))
    f.close()
...
def get_tls_payload(tls_payload, stream):
    ...
    if hasattr(tls_payload,'handshake_certificate_length'):
        stream['tls']['cert_length'] = tls_payload.handshake_certificate_length.all_fields[0].get_default_value()
    ...
    if hasattr(tls_payload,"handshake_type"):
        for record in tls_payload.handshake_type.all_fields:
            if record.get_default_value() == '1':
                    ...
                    stream['tls']['cipher_suite_length'] = tls_payload.handshake_cipher_suites_length
    return stream
def flatten(d, seperator = '_', new_dict = {}, parent_key = None):
    for key in d:
        if isinstance(d[key], dict):
            flatten(d[key],'_',new_dict, key)
        else:
            prefix = parent_key + "_" if parent_key else ""
            new_dict[prefix + key] = d[key]
    return new_dict
parse("./nontor.pcap")
# print(flatten(di))
